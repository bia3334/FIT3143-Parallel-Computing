...
int main(int argc, char *argv[]) {
    ...
    printf("CAAS Serial Prime Search - Searching for primes less than %d\n", n);
    ...
    FILE *outfile = fopen("primes_serial_caas.txt", "w");
    if (!outfile) {
    ...
    printf("\n=== CAAS SERIAL PERFORMANCE RESULTS ===\n");
    printf("Primes written to primes_serial_caas.txt\n");
    printf("Total primes found: %d\n", count);
    printf("Total execution time: %.6f seconds\n", elapsed_time);
    printf("Speedup: 1.000x (baseline)\n");
