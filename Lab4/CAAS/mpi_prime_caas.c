...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        printf("CAAS MPI Prime Search - n=%d, processes=%d\n", n, size);
    }
    ...
    if (rank == 0) {
        // Sort the gathered primes (due to cyclic distribution, they may be out of order)
        ...
        FILE *outfile = fopen("primes_mpi_caas.txt", "w");
        if (!outfile) {
        ...
        printf("\n=== CAAS MPI PERFORMANCE RESULTS ===\n");
        printf("Primes written to primes_mpi_caas.txt\n");
        ...
        // Non-root processes report their timing
        printf("Process %d: Compute=%.6fs, Comm=%.6fs\n", rank, compute_time, comm_time);
    }
